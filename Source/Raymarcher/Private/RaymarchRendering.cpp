...
IMPLEMENT_SHADER_TYPE(, FVolumePS, TEXT("/Plugin/Raymarcher/Private/RaymarchShader.usf"), TEXT("PassthroughPS"), SF_Pixel)
IMPLEMENT_SHADER_TYPE(, FAddOrRemoveDirLightShader, TEXT("/Plugin/Raymarcher/Private/DirLightShader.usf"), TEXT("MainComputeShader"), SF_Compute)
IMPLEMENT_SHADER_TYPE(, FChangeDirLightShader, TEXT("/Plugin/Raymarcher/Private/ChangeDirLightShader.usf"), TEXT("MainComputeShader"), SF_Compute)
IMPLEMENT_SHADER_TYPE(, FClearLightVolumesShader, TEXT("/Plugin/Raymarcher/Private/ClearVolumesShader.usf"), TEXT("MainComputeShader"), SF_Compute)
...
void WriteTo3DTexture_RenderThread(
	...
	// Set the bounds of where to write (you can write to multiple slices in any orientation - along X/Y/Z)
	...
	RHICmdList.CopyToResolveTarget(inTexture->Resource->TextureRHI, inTexture->Resource->TextureRHI, ResolveParams);

}

bool CreateVolumeTextureAsset(FString AssetName, EPixelFormat PixelFormat, FIntVector Dimensions, uint8* BulkData, bool SaveNow, bool UAVCompatible, UVolumeTexture** pOutCreatedTexture)
{
	...
	int TotalSize = Dimensions.X * Dimensions.Y * Dimensions.Z * GPixelFormats[PixelFormat].BlockBytes;
	...
	if (SaveNow) {
		...
		return UPackage::SavePackage(Package, NewTexture, EObjectFlags::RF_Public | EObjectFlags::RF_Standalone, *PackageFileName, GError, nullptr, true, true, SAVE_NoError);
	}
	else {
		return true;
	}
...
bool Create2DTextureAsset(FString AssetName, EPixelFormat PixelFormat, FIntPoint Dimensions, uint8* BulkData, bool SaveNow, TextureAddress TilingX, TextureAddress TilingY)
{
	ETextureSourceFormat TextureSourceFormat = PixelFormatToSourceFormat(PixelFormat);
	...
	UTexture2D* NewTexture = NewObject<UTexture2D>((UObject*)Package, FName(*AssetName), RF_Public | RF_Standalone | RF_MarkAsRootSet);
	...
	ERHIFeatureLevel::Type FeatureLevel)
{
	...
	if (LightParams.LightDirection == FVector(0.0, 0.0, 0.0)) {
		GEngine->AddOnScreenDebugMessage(-1, 100.0f, FColor::Yellow, TEXT("Returning because the directional light doesn't have a direction."));
		return;
	}
	...
	// Get Major Axes (notice inverting of light Direction - for a directional light, the position of the light is the opposite of the direction)
	// e.g. Directional light with direction (1, 0, 0) is assumed to be shining from (-1, 0, 0)
	...
	// Don't need barriers on these - we only ever read/write to the same pixel from one thread -> no race conditions
	// But we definitely need to transition the resource to Compute-shader accessible, otherwise the renderer might touch our textures while we're writing them.
	RHICmdList.TransitionResources(EResourceTransitionAccess::EWritable, EResourceTransitionPipeline::EGfxToCompute, UAVs, 4);
	...
	//	oldAxes.FaceWeight[0].second = newAxes.FaceWeight[0].second = 1;
		...
		FString text = "Adding/Removing light with new Major axis weight = " + FString::SanitizeFloat(axes.FaceWeight[0].second) + ", new second axis weight = " + FString::SanitizeFloat(axes.FaceWeight[1].second);
	...
	for (unsigned i = 0; i < passes; i++) {
		...
		FString text = addRemove + " Light @ Axis " + FString::SanitizeFloat(axes.FaceWeight[i].first, 0) + ", weight = " + FString::SanitizeFloat(axes.FaceWeight[i].second, 3) + 
			", pixel offset = " + FString::SanitizeFloat(textureOffset.X, 2) + ", " + FString::SanitizeFloat(textureOffset.Y, 2) + ", total alpha = " + FString::SanitizeFloat(alpha, 1);
		GEngine->AddOnScreenDebugMessage(-1, 100, (LightParams.Added ? FColor::Yellow : FColor::Red), text);
*/
		...
		FSamplerStateInitializerRHI init = FSamplerStateInitializerRHI(SF_Bilinear, AM_Border, AM_Border, AM_Border, 0, 0, 0, 0, colorInt);
		FSamplerStateRHIRef readBuffSampler = RHICreateSamplerState(init);

		// Create read-write buffer textures (with UAV createflag so we can use them in compute shaders).
		FRHIResourceCreateInfo CreateInfo(FClearValueBinding::Transparent);
		FTexture2DRHIRef Texture1 = RHICreateTexture2D(groups.X, groups.Y, PF_A32B32G32R32F, 1, 1, TexCreate_ShaderResource | TexCreate_UAV, CreateInfo);
		FTexture2DRHIRef Texture2 = RHICreateTexture2D(groups.X, groups.Y, PF_A32B32G32R32F, 1, 1, TexCreate_ShaderResource | TexCreate_UAV, CreateInfo);
		...
		ComputeShader->SetParameters(RHICmdList, LightParams, LightAdded, axes.FaceWeight[i].first, axes.FaceWeight[i].second);
		for (int j = 0; j < groups.Z; j++) {
			...
			if (j % 2 == 0) {
				...
				RHICmdList.TransitionResource(EResourceTransitionAccess::ERWBarrier, EResourceTransitionPipeline::EComputeToCompute, Texture2UAV);
				ComputeShader->SetLoop(RHICmdList, j, Texture1, readBuffSampler, Texture2UAV);
			//	ComputeShader->SetBorderSampler(RHICmdList, LightParams.LightColor, axes.FaceWeight[i].second * LightParams.LightIntensity);
			} else {
				RHICmdList.TransitionResource(EResourceTransitionAccess::EReadable, Texture2);
				RHICmdList.TransitionResource(EResourceTransitionAccess::ERWBarrier, EResourceTransitionPipeline::EComputeToCompute, Texture1UAV);
				ComputeShader->SetLoop(RHICmdList, j, Texture2, readBuffSampler, Texture1UAV);
			//	ComputeShader->SetBorderSampler(RHICmdList, LightParams.LightColor, axes.FaceWeight[i].second * LightParams.LightIntensity);
			}
			
			DispatchComputeShader(RHICmdList, *ComputeShader, groups.X / (NUM_THREADS_PER_GROUP_DIMENSION), groups.Y / NUM_THREADS_PER_GROUP_DIMENSION, 1);
		}
	}
	...
	RHICmdList.TransitionResources(EResourceTransitionAccess::EReadable, EResourceTransitionPipeline::EComputeToGfx, UAVs, 4);
}
	...
	ERHIFeatureLevel::Type FeatureLevel)
{
	...
	// If first major axes are the same and above the dominance threshold, ignore whether the second major axes are the same.
	if (oldAxes.FaceWeight[0].first != newAxes.FaceWeight[0].first ||
		(oldAxes.FaceWeight[1].first != newAxes.FaceWeight[1].first /*&& !(oldAxes.FaceWeight[0].second > dominance && newAxes.FaceWeight[0].second > dominance)*/)) {
		AddDirLightToLightVolume_RenderThread(RHICmdList, RLightVolumeResource, GLightVolumeResource, BLightVolumeResource, ALightVolumeResource, VolumeResource, TFResource, LightParams, false, FeatureLevel);
		AddDirLightToLightVolume_RenderThread(RHICmdList, RLightVolumeResource, GLightVolumeResource, BLightVolumeResource, ALightVolumeResource, VolumeResource, TFResource, NewLightParams, true, FeatureLevel);
		return;
	}
...
//	oldAxes.FaceWeight[0].second = newAxes.FaceWeight[0].second = 1;
	...
	FString text = "Moving light with new Major axis weight = " + FString::SanitizeFloat(newAxes.FaceWeight[0].second) + ", new second axis weight = " + FString::SanitizeFloat(newAxes.FaceWeight[1].second);
	GEngine->AddOnScreenDebugMessage(-1, 0, FColor::Yellow, text);

	for (unsigned i = 0; i < passes; i++) {

		FVector2D textureOffset = GetPixOffset(oldAxes.FaceWeight[i].first, -LightParams.LightDirection);
...
// Set the sampler for read buffer to use border with the proper light color.
		FSamplerStateInitializerRHI init = FSamplerStateInitializerRHI(SF_Trilinear, AM_Border, AM_Border, AM_Border, 0, 0, 0, 0, colorInt);
		FSamplerStateRHIRef readBuffSampler = RHICreateSamplerState(init);
		FSamplerStateInitializerRHI newinit = FSamplerStateInitializerRHI(SF_Trilinear, AM_Border, AM_Border, AM_Border, 0, 0, 0, 0, newColorInt);
		FSamplerStateRHIRef newReadBufferSampler = RHICreateSamplerState(newinit);

		// Create read-write buffer textures for the removed light (with UAV createflag so we can use them in compute shaders).
		FRHIResourceCreateInfo CreateInfo(FClearValueBinding::Transparent);
		FTexture2DRHIRef Texture1 = RHICreateTexture2D(groups.X, groups.Y, PF_A32B32G32R32F, 1, 1, TexCreate_ShaderResource | TexCreate_UAV, CreateInfo);
		FTexture2DRHIRef Texture2 = RHICreateTexture2D(groups.X, groups.Y, PF_A32B32G32R32F, 1, 1, TexCreate_ShaderResource | TexCreate_UAV, CreateInfo);
		// Create read-write buffers for the added light.
		FTexture2DRHIRef NewTexture1 = RHICreateTexture2D(groups.X, groups.Y, PF_A32B32G32R32F, 1, 1, TexCreate_ShaderResource | TexCreate_UAV, CreateInfo);
		FTexture2DRHIRef NewTexture2 = RHICreateTexture2D(groups.X, groups.Y, PF_A32B32G32R32F, 1, 1, TexCreate_ShaderResource | TexCreate_UAV, CreateInfo);
		...
		ComputeShader->SetParameters(RHICmdList, LightParams, NewLightParams, oldAxes.FaceWeight[i].first, oldAxes.FaceWeight[i].second, newAxes.FaceWeight[i].second);
		for (int j = 0; j < groups.Z; j++) {
			// Switch read and write buffers each cycle.
			if (j % 2 == 0) {
				RHICmdList.TransitionResource(EResourceTransitionAccess::EReadable, Texture1);
				...
				ComputeShader->SetLoop(RHICmdList, j, Texture1, readBuffSampler, Texture2UAV, NewTexture1, newReadBufferSampler, NewTexture2UAV);
			} else {
				RHICmdList.TransitionResource(EResourceTransitionAccess::EReadable, Texture2);
				...
				RHICmdList.TransitionResource(EResourceTransitionAccess::EWritable, EResourceTransitionPipeline::EComputeToCompute, NewTexture1UAV);
				ComputeShader->SetLoop(RHICmdList, j, Texture2, readBuffSampler, Texture1UAV, NewTexture2, newReadBufferSampler, NewTexture1UAV);
			}
	...
	ERHIFeatureLevel::Type FeatureLevel)
{
	...
	// Don't need barriers on these - we only ever read/write to the same pixel from one thread -> no race conditions
	// But we definitely need to transition the resource to Compute-shader accessible, otherwise the renderer might touch our textures while we're writing them.
	RHICmdList.TransitionResources(EResourceTransitionAccess::ERWNoBarrier, EResourceTransitionPipeline::EGfxToCompute, UAVs, 4);
	...
	DispatchComputeShader(RHICmdList, *ComputeShader, RLightVolumeResource->GetSizeX() / (NUM_THREADS_PER_GROUP_DIMENSION), RLightVolumeResource->GetSizeY() / NUM_THREADS_PER_GROUP_DIMENSION, 1);
