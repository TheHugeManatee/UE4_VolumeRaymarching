		...
		bool& LightAdded)
{
	...
	if (!DataVolume->Resource || !TransferFunction->Resource || !RLightVolume->Resource || !GLightVolume->Resource || !BLightVolume->Resource || !ALightVolume->Resource ||
		!DataVolume->Resource->TextureRHI || !TransferFunction->Resource->TextureRHI || !RLightVolume->Resource->TextureRHI || !GLightVolume->Resource->TextureRHI || !BLightVolume->Resource->TextureRHI || !ALightVolume->Resource->TextureRHI) {
		LightAdded = false;
		return;
	} else {
		LightAdded = true;
	...
	ENQUEUE_RENDER_COMMAND(CaptureCommand)(
		[RLightVolumeResource, GLightVolumeResource, BLightVolumeResource, ALightVolumeResource, VolumeResource, TFResource, LightParameters, Added, FeatureLevel](FRHICommandListImmediate& RHICmdList)
		{
			AddDirLightToLightVolume_RenderThread(
				RHICmdList,
				RLightVolumeResource,
				GLightVolumeResource,
				BLightVolumeResource,
	...
	bool& LightAdded)
{
	...
	// Call the actual rendering code on RenderThread.
	ENQUEUE_RENDER_COMMAND(CaptureCommand)(
		[RLightVolumeResource, GLightVolumeResource, BLightVolumeResource, ALightVolumeResource, VolumeResource, TFResource, OldLightParameters, NewLightParameters, FeatureLevel](FRHICommandListImmediate& RHICmdList)
	{
		ChangeDirLightInLightVolume_RenderThread(
			RHICmdList,
			RLightVolumeResource,
			GLightVolumeResource,
			BLightVolumeResource,
...
void URaymarchBlueprintLibrary::ClearLightVolumes(const UObject* WorldContextObject, UVolumeTexture* RLightVolume, UVolumeTexture* GLightVolume, UVolumeTexture* BLightVolume, UVolumeTexture* ALightVolume, FVector4 ClearValues /*= FVector4(0,0,0,0)*/)
{
	ERHIFeatureLevel::Type FeatureLevel = WorldContextObject->GetWorld()->Scene->GetFeatureLevel();
	...
	// Call the actual rendering code on RenderThread.
	ENQUEUE_RENDER_COMMAND(CaptureCommand)(
		[RLightVolumeResource, GLightVolumeResource, BLightVolumeResource, ALightVolumeResource, ClearValues, FeatureLevel](FRHICommandListImmediate& RHICmdList)
	{
		ClearLightVolumes_RenderThread(
			RHICmdList,
			RLightVolumeResource,
			GLightVolumeResource,
			BLightVolumeResource,
...
void URaymarchBlueprintLibrary::LoadRawVolumeIntoVolumeTexture(
	const UObject* WorldContextObject,
	FString textureName, FIntVector Dimensions, UVolumeTexture* inTexture) {
...
void URaymarchBlueprintLibrary::LoadRawVolumeIntoVolumeTextureAsset(const UObject* WorldContextObject, FString FileName, FIntVector Dimensions, FString TextureName, UVolumeTexture*& LoadedTexture)
{
	const int TotalSize = Dimensions.X * Dimensions.Y * Dimensions.Z;
	...
	bool Success = CreateVolumeTextureAsset(TextureName, PF_G8, Dimensions, TempArray, false, false, &OutTexture);
	if (Success) {
...
void URaymarchBlueprintLibrary::LoadMhdFileIntoVolumeTextureAsset(const UObject* WorldContextObject, FString FileName, FString TextureName, FIntVector& TextureDimensions, FVector& WorldDimensions, UVolumeTexture*& LoadedTexture)
{
	FString RelativePath = FPaths::ProjectContentDir();
	FString FullPath = IFileManager::Get().ConvertToAbsolutePathForExternalAppForRead(*RelativePath) + FileName;
	...
	TextureDimensions = info.Dimensions;
		...
		FString text = "Shit read from the MHD file :\nDimensions = " + FString::FromInt(info.Dimensions.X) + " " + FString::FromInt(info.Dimensions.Y) + " " + FString::FromInt(info.Dimensions.Z) + 
		"\nSpacing : " + FString::SanitizeFloat(info.Spacing.X, 3) + " "  + FString::SanitizeFloat(info.Spacing.Y, 3) + " "  + FString::SanitizeFloat(info.Spacing.Z, 3)+
		"\nFinal Dimensions : " + FString::SanitizeFloat(WorldDimensions.X, 3) + " "  + FString::SanitizeFloat(WorldDimensions.Y, 3) + " "  + FString::SanitizeFloat(WorldDimensions.Z, 3);
	GEngine->AddOnScreenDebugMessage(-1, 20.f, FColor::Yellow, text);

	LoadRawVolumeIntoVolumeTextureAsset(WorldContextObject, FileName.Replace(TEXT(".mhd"), TEXT(".raw")), info.Dimensions, TextureName, LoadedTexture);
	
	return;
}

void URaymarchBlueprintLibrary::TryVolumeTextureSliceWrite(
	const UObject* WorldContextObject,
...
void URaymarchBlueprintLibrary::ExportColorCurveToTexture(const UObject* WorldContextObject, UCurveLinearColor* Curve, int xDim, FString TextureName)
{
	...
	Create2DTextureAsset(TextureName, PF_FloatRGBA, FIntPoint(xDim, TextureHeight), (uint8*)samples, true);
...
void URaymarchBlueprintLibrary::CreateLightVolumeAsset(const UObject * WorldContextObject, FString TextureName, FIntVector Dimensions, UVolumeTexture *& CreatedVolume)
{
	EPixelFormat PixelFormat = PF_R16F;
	const unsigned TotalElements = Dimensions.X * Dimensions.Y * Dimensions.Z * GPixelFormats[PixelFormat].NumComponents;
	...
	UVolumeTexture* OutTexture = nullptr;
	bool Success = CreateVolumeTextureAsset(TextureName, PixelFormat, Dimensions, (uint8*)InitMemory, false, true,  &OutTexture);
	if (Success) {
		MY_LOG("Asset created and saved successfuly.")
	}
...
void URaymarchBlueprintLibrary::ReadTransferFunctionFromFile(const UObject* WorldContextObject, FString TextFileName, UCurveLinearColor*& OutColorCurve)
{
	FString RelativePath = FPaths::ProjectContentDir();
	FString FullPath = IFileManager::Get().ConvertToAbsolutePathForExternalAppForRead(*RelativePath) + TextFileName;
...
void URaymarchBlueprintLibrary::CustomLog(const UObject* WorldContextObject, FString LoggedString, float Duration)
{
